...
/**
 * integration testing class between ray constructing from camera and ray's intersections points with elements
 */
public class IntegrationTest {
     ...
     * camera for tests
     */
    private final Camera camera1 = new Camera(new Point(0,0,0),
            new Vector(0,0,-1),
            new Vector(0,-1,0))
            .setVPSize(3,3).setVPDistance(1);

    /**
     * another camera for tests
     */
    private final Camera camera2 = new Camera(new Point(0,0,0.5),
            new Vector(0,0,-1),
            new Vector(0,-1,0))
            .setVPSize(3,3).setVPDistance(1);
     ...
     * another camera for tests
     */
    private final Camera camera3 = new Camera(new Point(0,0,0),
            new Vector(0,1,0),
            new Vector(0,0,1))
            .setVPSize(3,3)
            .setVPDistance(1);
    ...
    /**
     * calculate the sum of the intersections points between camera and object
    ...
    public int numOfIntersections(Geometry object, Camera camera){
        ...
        for (int i=0;i<3;i++){
            for (int j=0; j<3;j++){
                List<Point> num = object.findIntsersections(camera.constructRay(3,3,j,i));
                if (num != null)
                    sum += num.size();
            }
        }
